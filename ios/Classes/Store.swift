...
class Store: ObservableObject {
    ...
    func listenForTransactions() -> Task<Void, Error> {
        return Task.detached {
            ...
            for await result in Transaction.updates {
                print("[IOS] Transaction.update: \(result)")
                do {
                    ...
                    print("[IOS] Transaction failed verification")
                }
            }
        }
    }
    ...
    func checkVerified<T>(_ result: VerificationResult<T>) throws -> T {
        print("[IOS] checkVerified \(result)")
    ...
    func updateCustomerProductStatus() async {
        print("[IOS] updateCustomerStatus")
        ...
        for await result in Transaction.currentEntitlements {
            print("[IOS] Transaction.currentEntitlements: \(result)")
            do {
                ...
                case .nonConsumable:
                    if let item = nonConsumables.first(where: { $0.id == transaction.productID }) {
                        purchasedNonConsumables.append(item)
                    }
                case .nonRenewable:
                ...
                case .autoRenewable:
                    print("[IOS] Subscriptions status: \(transaction.id)")
                    if let subscription = subscriptions.first(where: { $0.id == transaction.productID }) {
                        print("[IOS] Subscriptions status found: \(subscription)")
